...
@section('content')
    <form id="gameForm" action={{ route('data-update-game', ['id'=>$game->id]) }} class="p-0 lg:px-8 flex flex-col gap-8" method="POST">
        @csrf
        <div class="w-full">
            @if ($errors->any())
        ...
        <h1 class="text-4xl text-white font-light uppercase">EDIT GAME</h1>
        <div class="flex flex-col">
            <label for="game_title" class="text-gray-300 text-sm">Title</label>
            <input type="text" name="game_title" id="game_title" class="w-full md:max-w-full h-9 rounded-sm field-bg px-3 text-white @error('game_title') border border-[#C15755] @enderror" value="{{ old('game_title', $game->title) }}">
        </div>
        <div class="flex flex-col">
            <label for="game_brief_description" class="text-gray-300 text-sm">Brief Description</label>
            <textarea name="game_brief_description" id="game_brief_description" class="resize-none w-full h-32 rounded-sm field-bg px-3 py-2 text-white @error('game_brief_description') border border-[#C15755] @enderror">{{ old('game_brief_description', $game->brief_description) }}</textarea>
        </div>
        <div class="flex flex-col">
        ...
        <div class="flex flex-col">
            ...
            <input type="date" name="release_date" id="release_date" class="w-full md:max-w-80 h-9 rounded-sm field-bg px-3 text-white @error('release_date') border border-[#C15755] @enderror" value="{{ old('release_date', $game->released_date) }}">
        </div>
            @php
            $price = old('price', $game->price);
            if ($price) {
        ...
        <div class="flex flex-col">
            ...
            <input type="hidden" name="original_price" id="original_price" value="{{ old('original_price', $game->price) }}">
        </div>
        <div class="flex flex-col">
            <label for="discount_percentage" class="text-gray-300 text-sm">Discount Percentage</label>
            <input type="text" name="discount_percentage" id="discount_percentage" class="w-full md:max-w-80 h-9 rounded-sm field-bg px-3 text-white @error('discount_percentage') border border-[#C15755] @enderror" value="{{ old('discount_percentage', $game->discount_percentage) }}">
        </div>
        <div class="flex flex-col @error('age_rating') border border-[#C15755] @enderror">
            ...
            <div class="custom-radio-group">
                <input type="radio" id="pegi3" name="age_rating" value="1" class="custom-radio-input" {{ old('age_rating', $game->age_rating_id) == 1 ? 'checked' : '' }}>
            ...
            <div class="custom-radio-group">
                <input type="radio" id="pegi7" name="age_rating" value="2" class="custom-radio-input" {{ old('age_rating', $game->age_rating_id) == 2 ? 'checked' : '' }}>
            ...
            <div class="custom-radio-group">
                <input type="radio" id="pegi12" name="age_rating" value="3" class="custom-radio-input" {{ old('age_rating', $game->age_rating_id) == 3 ? 'checked' : '' }}>
            ...
            <div class="custom-radio-group">
                <input type="radio" id="pegi16" name="age_rating" value="4" class="custom-radio-input" {{ old('age_rating', $game->age_rating_id) == 4 ? 'checked' : '' }}>
            ...
            <div class="custom-radio-group">
                <input type="radio" id="pegi18" name="age_rating" value="5" class="custom-radio-input" {{ old('age_rating', $game->age_rating_id) == 5 ? 'checked' : '' }}>
        ...
        <div class="flex flex-col @error('genres') border border-[#C15755] @enderror">
            ...
            @foreach ($genres as $genre)
                <div class="custom-checkbox">
                    <input type="checkbox" id="genre{{$genre->id}}" name="genres[]" value="{{$genre->id}}" {{ in_array($genre->id, old('genres', $game->genres->pluck('id')->toArray())) ? 'checked' : '' }}>
        ...
        <div id="videoPreview" class="w-full">
            ...
            <video controls class="w-full rounded-sm">
                <source id="videoSource" src="{{ old('video_url', $game->trailer_url) }}" type="video/mp4">
        ...
        <div id="imagePreview" class="flex flex-wrap gap-4">
            @if(old('image_urls', $game->game_images))
            @foreach(old('image_urls', $game->game_images) as $image)
                <div class="relative inline-block">
                    @if(is_object($image))
                        <img src="{{$image->image_url}}" class="w-56 h-36 object-cover rounded-sm border border-gray-300">
                        <input type="hidden" name="image_urls[]" value="{{$image->image_url}}">
                    @else
                        <img src="{{$image}}" class="w-56 h-36 object-cover rounded-sm border border-gray-300">
        ...
        <input type="hidden" name="video_url" id="video_url" value="{{ old('video_url', $game->trailer_url) }}">
