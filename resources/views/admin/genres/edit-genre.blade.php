...
@section('content')
    <form action={{ route('data-genre-update', ['id' => $genre->id]) }} class="p-0 lg:px-8 flex flex-col gap-8" method="POST">
        @csrf
        @method('PUT')
        <div class="w-full">
            @if ($errors->any())
        ...
        <h1 class="text-4xl text-white font-light">EDIT GENRE</h1>
        <div class="flex flex-col">
            <label for="genre_name" class="text-gray-300 text-sm">Genre Name</label>
            <input type="text" name="genre_name" id="genre_name" class="w-full md:max-w-80 h-9 rounded-sm field-bg px-3 text-white @error('genre_name') border border-[#C15755] @enderror" value="{{ old('genre_name', $genre->name) }}">
        </div>
